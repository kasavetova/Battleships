...
public class Player extends JFrame implements ActionListener{
                    ...
                    try {
                        ...
                        while ((input = (Request) in.readObject()) != null) {
                            if (input.getActionType().equals("UserJoinedLobby")) {
                                if(!input.getObject().equals(name)) { playersModel.addElement((String) input.getObject()); }
                                ...
                                if (input.getActionType().equals("GameRequest")) {
                                    //check if there is already a dialog box opened.
                                    //if user accepts one of the game requests - the other dialogs are closed
                                    //and an answer 'no' is sent to the server
                                    int requestAnswer = JOptionPane.showConfirmDialog(null, "Do you want to play a game with "
                                            + input.getOrigin() + "?", "Game Request", JOptionPane.YES_NO_OPTION);
                                    //no = 1, yes = 0
                                    if (requestAnswer == 1) {
                                        out.writeObject(new Request("GameRequestAnswer", name, input.getOrigin(), "No"));
                                    }
                                    if (requestAnswer == 0) {
                                        out.writeObject(new Request("GameRequestAnswer", name, input.getOrigin(), "Yes"));
                                        gameFrame(input.getOrigin());
                                        opponentName = input.getOrigin();
                                    }
                                } else if (input.getActionType().equals("GameRequestAnswer")) {
                                    if (input.getObject().equals("Yes")) {
                                        gameFrame(input.getOrigin());
                                        opponentName = input.getOrigin();
                                    } else if (input.getObject().equals("No")) {
                                        JOptionPane.showMessageDialog(null, "Game request denied from " + input.getOrigin() + ".",
                                    ...
                                    for (int i=0; i<playersList.size(); i++){
                                        if(!playersList.get(i).equals(name)){
                                            ...
                                            playersModel.addElement(playersList.get(i).toString());
                                        }
                                    }
                                } else if(input.getActionType().equals("ReceiveMessage")) {
                                    String originalText = chatArea.getText().replaceAll("<html>", "").replaceAll("</html>", "");
    ...
    public void mainGUI() {
        ...
        playRandButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    System.out.println("CATCH FROM RANDOM BTN" + e1.getMessage());
                }
            }
        });
        ...
        players.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent evt) {
                //need to disable clicked item until a response has been received.
                if (evt.getClickCount() == 2) {
                    String playerName = playersModel.getElementAt(players.getSelectedIndex());
                    if (!(name.equals(playerName))) {
                        try {
                            out.writeObject(new Request("GameRequest", name, playerName));
                        }
                        catch (IOException e) {
                            e.printStackTrace();
                        }
    ...
    public void gameFrame(String opponentName) {
        ...
        east.add(chatArea, BorderLayout.CENTER);
        eastSouth.add(chatMessage, BorderLayout.CENTER);
        eastSouth.add(sendButton, BorderLayout.EAST);
        ...
        chatMessage.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if(chatMessage.getText().length()>0) {
                    String originalText = chatArea.getText().replaceAll("<html>", "").replaceAll("</html>", "");
                    String newText = null;
                    if(originalText.indexOf("<br>")==0) {originalText.replace("<br>", "");}
                    if(originalText.length()==0) { newText = "<html>" + originalText + name + ": " + chatMessage.getText() + "</html>"; } //this is to remove
                    else { newText = "<html>" + originalText + "<br><b>" + name + ":</b> " + chatMessage.getText() + "</html>"; }                //br tags at start of message
                    chatArea.setText(newText);
                    try {
                        out.writeObject(new Request("SendMessage", name, opponentName, chatMessage.getText()));
                    } catch (IOException e1) {
                        e1.printStackTrace();
                    }
                    chatMessage.setText("");
